...
WITH ACCOUNT_NUMBER_DETAIL AS (
           SELECT 
    IDD,
    LISTAGG(ACCOUNT_NO, ':') WITHIN GROUP (ORDER BY ACCOUNT_NO) AS ACCOUNT_NO,
    LISTAGG(REG_NAME, ':') WITHIN GROUP (ORDER BY REG_NAME) AS COMPANY_NAME
FROM (
   SELECT   ASM.REG_CODE AS ACCOUNT_NO,
         AFLD.IDS AS IDD,
         ASRC.REG_NAME
     
    FROM 
            AB_SETUP_REGISTRATION ASM
      JOIN AB_SETUP_REGISTRATION ASRC ON ASRC.SR_ID=ASM.COMPANY_ID AND ASRC.REG_TYPE='COMPANY'
       JOIN AB_FACILITIES_LETTER_DET AFLD ON  AFLD.ACCOUNT_NO = ASM.SR_ID 
    WHERE 
        -----    ASM.REG_TYPE='BANK LOV'
        AFLD.IMAGE_TYPE = 'FACILITIES ACCOUNT NUMBER'
...
select AFL.ID,
    ...
    MAX(
        ...
        END) AS NATURE_NAME,
       AFL.NATURE_ID,
       TO_CHAR(AFL.AMOUNT,'999G999G999G999G990D00') AMOUNT,
       --TO_CHAR(AFL.CASH_MARGIN,'999G999G999G999G990D00') CASH_MARGIN,
       AFL.cash_margin,
       AFL.ISSUE_DATE,
       AFL.EXP_DATE,
       AFL.SERVICE_CHARGES,
       AFL.BG_SECURITY,
       ACCNT.ACCOUNT_NO AS ACCOUNT_NOOO,
      --------- AFL.ACCOUNT_NO,
       AFL.COLLATERAL,
       AFL.BG_CHARGES,
       AFLS.IDS,
  ...
  --  LISTAGG(DISTINCT CASE WHEN AFLS.COMPANY_NAME = ASR.SR_ID AND AFLS.IMAGE_TYPE = 'FACILITIES COMPANY' THEN ASR.REG_NAME END, ': ')  WITHIN GROUP (ORDER BY AFLS.IDS) AS COMPANY_NAMES,
    ...
    LISTAGG(DISTINCT CASE WHEN AFLS.INTITLED_COMPANY = ASR.SR_ID  THEN ASR.PARTY_NAME  END, ': ') 
      ...
      -- MAX(CASE WHEN ASR.SR_ID = AFLS.COMPANY_NAME AND REG_TYPE = 'COMPANY' THEN ASR.REG_NAME ELSE NULL END ) AS COMPANY_NAMES,     
       ...
       ACCNT.COMPANY_NAME COMPANY_NAMES



 from AB_FACILITIES_LETTER_DET AFL
 JOIN AB_FACILITIES_LETTER_DET AFLS ON AFLS.IDS = AFL.ID
LEFT JOIN ACCOUNT_NUMBER_DETAIL   ACCNT   ON ACCNT.IDD = AFL.ID
